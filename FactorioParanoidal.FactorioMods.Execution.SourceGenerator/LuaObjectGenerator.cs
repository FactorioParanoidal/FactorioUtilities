    ...
    public class LuaObjectGenerator : IIncrementalGenerator {
        public void Initialize(IncrementalGeneratorInitializationContext context) {
            context.RegisterPostInitializationOutput(ctx => {
                ctx.AddSource("GeneratePrototypesAttribute.g.cs", SourceText.From(@"
using System;
namespace FactorioParanoidal.FactorioMods.Execution.SourceGenerator
{
...
namespace FactorioParanoidal.FactorioMods.Execution.SourceGenerator
{
    ...
    internal class LuaExtraFieldsAttribute : Attribute { }
            ...
            context.RegisterSourceOutput(jsonProvider, static (spc, json) => {
                var generator = new PrototypeGenerator();
                generator.Execute(json!, spc);
            });
        ...
        private static void Execute(Compilation compilation, ImmutableArray<TypeDeclarationSyntax?> classes,
            ...
            foreach (var classDeclaration in classes) {
                ...
                // PrototypeGenerator classes are in namespace FactorioParanoidal.FactorioMods.Execution.Prototypes
                // and they already implement ILuaUserData.
                if (classSymbol.ContainingNamespace.ToDisplayString() ==
                    "FactorioParanoidal.FactorioMods.Execution.Prototypes") {
                    // Check if it's one of the manually created ones we want to process
                    // (GenericPrototype and PrototypeBase are manual but now handled manually)
                    if (classSymbol.Name != "GenericPrototype" && classSymbol.Name != "PrototypeBase") {
                        // Most likely a generated prototype, skip to avoid duplicates or conflicts
                        // if it was already generated by PrototypeGenerator in this same project.
                        // Actually, LuaObjectGenerator only sees what's in the compilation.
                        // If it's partial and exists in source, we might still want to generate Lua bindings
                        // BUT our PrototypeGenerator already generates the Lua bindings for its types.
                        continue;
                    }
                }
