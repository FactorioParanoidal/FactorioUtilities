...
public class FactorioLuaEngineTests : IDisposable
{
    ...
    {
        // Arrange
        var info = new FactorioModInfo { Name = "test-mod", Version = new Version(1, 0, 0), Title = "test", Author = "test" };
        ...
        var item = engine.Registry.Prototypes["item"]["test-item"].Read<LuaTable>();
        item["stack_size"].ToString().Should().Be("50");
    }
    ...
    public async Task RunAllStages_HandlesMultiStageOverrides()
    {
        ...
        // Assert
        var item = engine.Registry.Prototypes["item"]["test-item"].Read<LuaTable>();
        item["value"].ToString().Should().Be("2");
    }
}
