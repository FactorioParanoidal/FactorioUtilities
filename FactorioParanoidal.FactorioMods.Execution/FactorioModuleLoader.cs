...
public class FactorioModuleLoader : ILuaModuleLoader
{
    ...
    private string? ResolvePath(string moduleName)
    {
        ...
        {
            int secondDoubleUnderscore = path.IndexOf("__", 2);
            if (secondDoubleUnderscore != -1)
            {
                string modName = path.Substring(2, secondDoubleUnderscore - 2);
                string subPath = path.Substring(secondDoubleUnderscore + 2).TrimStart('/');
