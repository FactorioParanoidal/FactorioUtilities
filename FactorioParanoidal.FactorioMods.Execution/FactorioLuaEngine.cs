...
public class FactorioLuaEngine : IDisposable
{
    ...
    private void SetupEnvironment()
    {
        ...
        dataTable["raw"] = _registry.ToLuaTable(_state);

        // data:extend(table)
        dataTable["extend"] = new LuaFunction(async (LuaFunctionExecutionContext context, CancellationToken cancellationToken) =>
        {
            int tableArgIndex = context.ArgumentCount > 1 ? 1 : 0;
            if (context.ArgumentCount > tableArgIndex && context.GetArgument(tableArgIndex).TryRead<LuaTable>(out var t))
            {
                _registry.Extend(t);
                dataTable["raw"] = _registry.ToLuaTable(_state);
            }
        ...
        _state.Environment["log"] = new LuaFunction(async (LuaFunctionExecutionContext context, CancellationToken cancellationToken) =>
        {
        ...
        _state.Environment["table_size"] = new LuaFunction(async (LuaFunctionExecutionContext context, CancellationToken cancellationToken) =>
        {
    ...
    private LuaTable CreateSerpentMock()
    {
        ...
        serpent["dump"] = new LuaFunction(async (LuaFunctionExecutionContext context, CancellationToken cancellationToken) =>
        {
            if (context.ArgumentCount > 0) return context.Return(context.GetArgument(0).ToString());
            return context.Return("");
        });
        serpent["line"] = new LuaFunction(async (LuaFunctionExecutionContext context, CancellationToken cancellationToken) =>
        {
            if (context.ArgumentCount > 0) return context.Return(context.GetArgument(0).ToString());
            return context.Return("");
        });
        serpent["block"] = new LuaFunction(async (LuaFunctionExecutionContext context, CancellationToken cancellationToken) =>
        {
            if (context.ArgumentCount > 0) return context.Return(context.GetArgument(0).ToString());
            return context.Return("");
        });
